...
## CVE-2026-59870 (js-yaml `!!omap` quadratische CPU-Auslastung) – PR #140, gemerged 2026-08-30

Dependabot-Alarm gemeldet, aber trotz bestehendem `overrides`-Eintrag (`^4.2.0` seit PR #83, siehe Issue #52) nicht abgedeckt: das Lockfile löste noch auf js-yaml **4.3.0** auf. Laut Advisory (GHSA-5p4m-2wfm-xmqj) wurde der Fix erst in **4.3.1** in die 4.x-Reihe zurückportiert – 4.3.0 blieb verwundbar; 3.x hat laut Advisory-Titel gar keinen Backport bekommen, 5.x ist seit 5.2.1 sicher.
...
## Dependabot-Fehlschlag @xmldom/xmldom – scoped Override (PR #154, gemerged 2026-09-02)

Der Dependabot-Lauf auf `main` (Run 33674650883) brach mit `update_files`-Fehler ab. Ursache laut Log: für `@xmldom/xmldom` existiert ein Patch, aber der von npm gewählte Update-Pfad hätte `@expo/cli` von 57.0.20 auf **0.0.0** downgraded. Also erneut der `overrides`-Fall aus CLAUDE.md (kein SDK-Downgrade).

- **Fix**: Override **scoped** statt global: `"plist": { "@xmldom/xmldom": "^0.9.12" }`. Grund: `plist@3.1.1` verlangt `^0.9.10` (0.9.10 → 0.9.12, Advisory-Range `>=0.9.0 <=0.9.11`), während `@expo/plist@0.8.1` mit `^0.8.8`/0.8.15 **außerhalb** der Advisory-Range liegt. Ein globaler Override hätte letzteres unnötig über die Major-Grenze 0.8 → 0.9 gehoben.
- **Lektion (neu)**: Overrides möglichst auf den betroffenen Consumer scopen (`{"parent": {"dep": "version"}}`), nicht global. Vorher prüfen, ob *alle* Konsumenten wirklich in der Advisory-Range liegen – hier war nur einer von zweien betroffen.
- **`decode-uri-component` bewusst NICHT gepinnt**: Die einzige gepatchte Version (0.5.0) ist **ESM-only** (`type: module`), `query-string@7.1.3` lädt sie per `require()`. Empirisch verifiziert – mit Override bricht `query-string.parse()` mit `TypeError: decodeComponent is not a function`. Da `expo-router` `query-string` fürs Deep-Link-/URL-Parsing nutzt, wäre das ein **Runtime-Breakage der Navigation** gegen eine moderate DoS-Advisory in einer Build-/Dev-Kette. 0.4.2 wäre wieder CJS, liegt aber selbst noch in der Range (`<=0.4.2`). Bleibt offen, bis `expo-router` auf `query-string >= 10` geht.
- **Lektion (neu)**: Vor einem Override immer prüfen, ob die gepatchte Version das Modulformat wechselt (`npm view <pkg>@<v> type exports`). ESM-only-Patches sind für CJS-Konsumenten ein stiller Runtime-Breaker, den `npm audit`/`tsc`/Jest **nicht** fangen – hier war ein expliziter `node -e "require(...)"`-Test nötig.
